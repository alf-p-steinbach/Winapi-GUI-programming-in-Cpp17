...
Okay that was five major differences, I lied. But. And (editing this many days later!) I now recall that there’s one more and practically important difference: in a batch file, if there’s no environment variable `foo` then `%foo%` expands to nothing, an empty string, while in an interactive command it expands to `%foo%`.
...
&nbsp;&nbsp;&nbsp;&nbsp; <img src="02/images/hello.fullname.png">

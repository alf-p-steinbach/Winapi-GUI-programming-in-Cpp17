...
- [Chapter 2. An introduction to command line work.](#chapter-2-an-introduction-to-command-line-work)
  ...
  - [2.2. Command line concepts & basic usage / MinGW g++.](#22-command-line-concepts--basic-usage--mingw-g)
    ...
    - [2.2.5. Let’s install the MSYS2 g++ compiler.](#225-lets-install-the-msys2-g-compiler)
      ...
      - [Deal with Windows’ FUD security warnings.](#deal-with-windows-fud-security-warnings)
      - [Guess right about whether a specified installation directory will be used directly or just as a parent directory.](#guess-right-about-whether-a-specified-installation-directory-will-be-used-directly-or-just-as-a-parent-directory)
      - [Wintty console windows are (still) a thing.](#wintty-console-windows-are-still-a-thing)
      - [Use MSYS2’s package manager `pacman` to install g++.](#use-msys2s-package-manager-pacman-to-install-g)
    ...
    - [2.2.7. Let’s make MSYS2’s g++ available in Cmd.](#227-lets-make-msys2s-g-available-in-cmd)
      ...
      - [Unexpected: DLL not found and three g++ bugs. As if one wasn’t enough.](#unexpected-dll-not-found-and-three-g-bugs-as-if-one-wasnt-enough)
    ...
    - [2.2.8. Let’s build the GUI “Hello, world!” program with g++.](#228-lets-build-the-gui-hello-world-program-with-g)
      - [Building with console subsystem is maximally simple.](#building-with-console-subsystem-is-maximally-simple)
      - [With a console subsystem executable Cmd waits for program completion.](#with-a-console-subsystem-executable-cmd-waits-for-program-completion)
      - [Check the subsystem with the MinGW tools.](#check-the-subsystem-with-the-mingw-tools)
      - [Building with GUI subsystem is also easy.](#building-with-gui-subsystem-is-also-easy)
    ...
    - [2.2.9. Let’s create a batch file to set up the `PATH` etc. for g++.](#229-lets-create-a-batch-file-to-set-up-the-path-etc-for-g)
      ...
      - [Add compiler configuration and an alias to the `PATH`-fixing batch file.](#add-compiler-configuration-and-an-alias-to-the-path-fixing-batch-file)
      - [Cmd uses `^` as an escape character.](#cmd-uses-%5E-as-an-escape-character)
      - [Testing is always a good idea.](#testing-is-always-a-good-idea)
      - [Personal tools versus tools made for use by others.](#personal-tools-versus-tools-made-for-use-by-others)
...
<sup>in *section 2.2. “Command line concepts & basic usage”*</sup>  
<sup>in *chapter 2. “An introduction to command line work”*</sup>
...
Oh, the `findstr "\..."`. It passes on just the lines that contain a period followed by at least two non-space characters, such as in a filename extension, and discards the rest, the noise from `dir`. Unlike `find` the `findstr` pattern is not a literal string but a pattern with wildcards, where unescaped `.` stands for a non-space character.
...
This is of course a candidate for a batch file or Cmd alias (can be defined with `doskey`).

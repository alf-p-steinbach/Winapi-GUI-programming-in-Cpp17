...
`WM_SIZE` is a message that has parameters, values encoded in `w_param` and `ell_param`. They’re just ignored here. But when we start using \<windowsx.h\> message cracker macros the message handler functions need to have the appropriate parameters that the macros generate arguments for.
...
This is intentionally *identical* to the result before the change to `Polyline`: identical result increases confidence that the rewrite was correct.

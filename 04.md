...
A successful `BeginPaint` call produces a handle to a **device context**, which is like a canvas to paint on; the result ends up in the window’s client area. But this is a C API so painting calls look like `Ellipse( dc, ... )` instead of C++-ish `dc->Ellipse( ... )`. Even though the device context handle technically is a pointer it’s just a value that *identifies* the device context.

In addition to providing the pixels for the generated graphics the device context provides automatic **clipping** (and much more, but more about that later). The clipping ensures that no matter what your `paint` code does it won’t affect anything outside the window’s client area. More precisely it won’t affect anything outside the **update rectangle** for the current `BeginPaint` call, the area that needs to be regenerated, so except for efficiency considerations you can let your graphics operations do just about anything, anywhere. The update rectangle is available in the `PAINTSTRUCT` and can in principle be used to limit the graphics regeneration, for efficiency. However I cannot remember that I’ve ever needed that, so only the `dc` is passed to `paint`.

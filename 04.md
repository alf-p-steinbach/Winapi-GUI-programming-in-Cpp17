...
#### 4.2.2. Using `PolyLine` for simpler and faster drawing.
...
So let’s do it, let’s replace `MoveToEx` + `LineTo` with a single call to `PolyLine`.

Happily C++ has `std::vector` for storing the **`POINT`**s in an array, as `PolyLine` requires:

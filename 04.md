...
Here the supporting type `Px_index`, used for clarity, to guarantee no conflation of pixel indices with other coordinates, and to support overloading for e.g. `.px_pt_from`, is a strongly typed restricted integer-like type defined – unfortunately rather awkwardly, due to missing language support – by
...
*  to convert from math to indices  
  `.px_i_first`, `.px_i_beyond` and `.px_index_from_math_y`;

* to convert from index + axis to math (the axis is specified in the function name)  
  `.math_x_from` ; and

* just a type conversion *with possible mirroring*, individual pixel indices → `POINT`,  
  `.px_pt_from_indices`.

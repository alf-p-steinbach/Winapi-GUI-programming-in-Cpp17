...
Fine point: [the documentation](https://learn.microsoft.com/en-us/windows/win32/api/wingdi/nf-wingdi-polyline) is silent on whether `Polyline` draws the last point. In my experimentation, with my Windows version and default device context, it did *not* draw the last point. We’ll need to deal with that for drawing ticks on the math axes because one pixel versus two pixel length, while ordinarily not very visible, is a visually significant difference for line segments.

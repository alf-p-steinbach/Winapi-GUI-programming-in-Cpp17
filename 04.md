...
The client area size is really all that’s needed to compute all the local variables of the former v4 `paint`. The `Painter` constructor could alternatively itself have obtained that information from the device context by assuming that it’s a device context for a window and using the **`WindowFromDC`** function to get a handle to the window, and then `GetClientRect`. However, then it couldn’t be used to paint into a bitmap image, via device context that represents an image, whereas a `Painter` that doesn’t assume a window — and therefore gets a separate `SIZE` — can be used also to paint to a bitmap image.

It turns out that with graph-orientation agnostic code there is no need to have separate function for drawing the *x*-axis and the *y*-axis. And ditto for placing ticks on the axes. So instead of the decidedly un-DRY four original code snippets we end up with just two member functions `draw_math_axis` and `add_math_axis_ticks`, like this:
...
As shown this code assumes an **axis enumeration** type `Ct::Math_axis::Enum`, defined simply as

...
Given the size of that code there is some refactoring on the horizon, but for now, result:
...
For the coordinate transformation class we’ll discover what’s required.

Already: that it uses an **axis enumeration** type `Ct::Math_axis::Enum`, which is defined simply as
...
Recall that a C++ reference is an alias for something, an alternative name. I added the **name alias** `_` to reduce visual noise from “`m_transform`” peppered everywhere. An alternative could have been to use private inheritance of the coordinate transformation class, and that would totally get rid of the qualification noise, but at the cost of possibly making the purpose and responsibility of `Painter` less clear.

And in passing, the `draw_line` call implies that `.px_pt_from` returns a type at least convertible to GDI `POINT`.

In fact it just returns `POINT` but using a name alias `Px_point` because I don’t like all that uppercase everywhere, and also to tie a connection to `Px_point_vector` (discussed later). So this code uses name aliases both to reduce noisy verbosity, to get rid of all uppercase shouting, and to hint about connections, which are clear advantages. But still that is perhaps borderline like a Russian novel with umpteen names for each person; it’s maybe even crossing the line.
...
The chances of actual reuse are slim to non-existent for `Px_point_vector_`, but decoupling from the Windows API generally clarifies things. One can avoid some Microsoft complications, and some other complications can become more visible, explicitly dealt with, which generally is a Good Thing™. Here there were two complications of the latter category:
...
The v4 plotting code was pixel based, looping over the pixels on the math *x* axis.

With knowledge of the graph orientation the math *x* value could then be calculated directly from the pixel index,
...
And then the pixel column index could be calculated directly from the corresponding math *y* value,
...
Or in full, but omitting the supporting variable definitions:
...
To express this without knowledge of the graph orientation one most naturally ends up transforming (x,&nbsp;y) coordinate pairs instead of single scalars. E.g. math *x* would be computed from a GDI (column-index, row-index) pair. Which would be increased verbosity and possible reduced efficiency, an **abstraction cost**.

One solution is to recognize and directly support the hybrid notion of “pixel index on a math axis”.

The code then has to deal with only a single scalar, namely the pixel index, plus a binary choice of which axis:
...
Here the supporting type `Px_index`, used to guarantee no conflation of pixel indices with other coordinates, is a strongly typed restricted integer type defined (unfortunately rather awkwardly, due to missing language support) by
...
… and the support functions are `.px_i_first`, `.px_i_beyond` and `.px_index_from_math_y` to convert from math to indices; `.math_x_from` to convert from index + axis to math (the axis is specified in the function name); and `.px_pt_from_indices` which is just a type conversion *with possible mirroring*, individual pixel indices → `POINT`.

All this was present in the v4 code as the concepts behind its direct calculations: now they’re explicit and named.

`.px_pt_from_indices` could have been more type safe with distinct types for the indices for the *x*  and *y* axes. So why is there just a single general `Px_index` type? Well, with distinct types it would be more challenging to loop over the two axes, or to generally treat the axis as a parameter. For example, the function
...
… would then have to be expressed as a pair of functions, one for each axis and return type. And then code for looping over the axes to do something would have to be expressed with templating.

Remarkably the code to add markers on the graph doesn’t need pixel indexing and doesn’t even need to use rotation,
